#!/bin/bash
# Historical benchmark ladder with 9b7d54a broken into individual stages.
...
# Usage: bash test/run_pmu_ladder_v2.sh

set -e

if [ "$(id -u)" -eq 0 ]; then
  echo "error: do not run test/run_pmu_ladder_v2.sh with sudo or as root." >&2
