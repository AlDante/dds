/**
 * @file alpha_mu_core.h
 * @brief Shared data structures and callable entry points for the alpha-mu solver.
 *
 * This implementation follows the two-paper alpha-mu line described in
...
#ifndef DDS_TEST_ALPHA_MU_CORE_H
#define DDS_TEST_ALPHA_MU_CORE_H
...
namespace alpha_mu
{
   ...
   * the core set representation used throughout the alpha-mu solver.
   *
   ...
   *
   * The toy tree lets alpha-mu test non-locality, early cut, cut-on-win,
  ...
  /** @brief Canonical seat numbering used throughout alpha-mu. */
  enum Seat
  {
    SEAT_NORTH = 0,
    SEAT_EAST = 1,
  ...
  /** @brief Canonical suit numbering used throughout alpha-mu. */
  enum SuitIndex
  {
    SUIT_SPADES = 0,
    SUIT_HEARTS = 1,
  ...
  /**
   * @brief Bridge continuation state searched by alpha-mu.
   *
  ...
  /** @brief Prefix used by alpha-mu status and failure messages. */
  extern const char kAlphaMuMessagePrefix[];
  ...
  /** @brief Abort the current run with an alpha-mu-prefixed fatal message. */
  void Fail(const string& msg);
  /** @brief Assert an alpha-mu invariant and fail with context if it is violated. */
  ...
  /** @brief Convert PBN seat letters to the alpha-mu seat indices. */
  ...
  /** @brief Convert one parsed DDS deal into the alpha-mu bridge-state wrapper. */
  ...
  /** @brief Print a successful alpha-mu status line with the standard prefix. */
