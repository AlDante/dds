...
## 10. Thread-Safety and Reentrancy Analysis

DDS runs multiple search threads in parallel, each with its own `ThreadData` but sharing read-only deal data. Several of the optimisations above affect thread-safety or reentrancy. This section flags each risk.

### Safe — No Thread-Safety Impact

| Optimisation | Why Safe |
|---|---|
| Branch hints (§1) | Compiler hints only; no data changes |
| NEON for 4-suit loops (§2) | Same writes to same thread-local arrays, just vectorised |
| Prefetch (§4) | Advisory only; CPU ignores if inappropriate |
| P-core pinning (§5) | Thread scheduling only; no shared data |
| PGO + LTO (§6, §7) | Compiler optimisations; no semantic changes |
| Co-locate hot fields in `pos` (§8.5) | Layout change only; no aliasing change |
| Precompute booleans (§9.3) | Local variables only |
| CLZ intrinsic (§9.4) | Pure function replacing pure table lookup |
| NEON ranks accumulation (§9.5) | Read-only OR across `rankInSuit` |
| Return struct (§9.6) | Eliminates shared `int& res`; *improves* safety |
| SecondHand flatten (§9.7) | Same operations, different encoding |

### Requires Care

#### Cache-line alignment (§3) — False Sharing Risk

Aligning structs to 128 bytes is correct, but if two `ThreadData` instances end up *adjacent* in memory without padding, the last field of one and the first field of the next may share a cache line. On M1 Max (128-byte lines), this risk is doubled compared to Intel.

**Mitigation:** Allocate `ThreadData` instances with `aligned_alloc(128, sizeof(ThreadData))` or ensure the struct size is a multiple of 128 bytes:

```cpp
struct alignas(128) ThreadData {
    // ...
    char _pad[128 - (sizeof(ThreadDataHot) + sizeof(ThreadDataCold)) % 128];
};
```

#### Hot/cold split of `ThreadData` (§8.1) — Pointer Aliasing

Splitting `ThreadData` into `ThreadDataHot` and `ThreadDataCold` is safe **provided** both sub-structs remain within the same per-thread allocation. If they are separated into independent allocations, any code that currently passes `ThreadData&` will need updating. The sub-functions in QuickTricks take `const ThreadData& thrd` — after the split, they must take `const ThreadDataHot&` or the full `ThreadData&` depending on which fields they access.

**Risk:** `QuickTricksPartnerHandTrump` and `QuickTricksPartnerHandNT` access `thrd.rel[ranks]` and `thrd.nodeTypeStore` — if `rel` ends up in `ThreadDataCold` but is accessed on every node, it must stay in `ThreadDataHot`. Profile which fields are actually hot before splitting.

#### SoA transposition table (§8.2) — Concurrent Probe/Store

If DDS uses lock-free concurrent access to the transposition table (common in game-tree search), splitting keys and payloads into separate arrays introduces a **torn read** risk: a thread may read a key that matches, but by the time it reads the corresponding payload, another thread has overwritten that slot.

**Mitigation:** This is the same race that exists with AoS layouts (the key and value are not atomically written together), but SoA makes it more explicit. Options:
- Use a generation counter or versioned lock per slot
- Accept rare incorrect reads (DDS may already tolerate this — the search is self-correcting since wrong TT entries just cause extra work, not incorrect results)
- Use `std::atomic<uint64_t>` for keys with `memory_order_acquire`/`release`

**If DDS currently uses Zobrist-based lockless hashing** (XOR key with value for storage, XOR back on read to verify), this technique must be adapted for SoA since the key and value are no longer adjacent.

#### Depth-local scratch pad (§8.3) — Stack Depth

Moving `winRanks` and `lowestWin` from indexed arrays to stack-allocated `DepthLocal` structs is thread-safe by construction (each thread has its own stack). However:

- **Stack size:** 49 depths × 48 bytes = 2.4 KB per thread, well within default stack limits. Safe.
- **Reentrancy:** If `QuickTricks` or other functions access `tpos.winRanks[depth]` directly, they must be refactored to use the `DepthLocal*` pointer instead. Any code path that writes to `tpos.winRanks[depth]` (QuickTricks does this extensively) must be updated or the `DepthLocal` must be copied back to `tpos.winRanks[depth]` before returning.

**This is the highest-risk refactor** — `tpos.winRanks[depth]` is written in QuickTricks, ABsearch, and the Make/Undo functions. All must agree on whether the source of truth is the `DepthLocal` or the `pos` struct.

#### `moveType` packing (§8.4) — Alignment and Atomicity

Packing `moveType` to 4 bytes is safe for single-threaded access. However, if any code reads/writes `moveType` across threads (e.g., `bestMove` shared between helper threads), sub-word writes on ARM64 are not guaranteed atomic for packed structs.

**Mitigation:** `bestMove` is per-thread (`thrp->bestMove[depth]`), so this is safe in DDS's current architecture. Just ensure no `moveType` is shared across threads without synchronisation.

#### QuickTricks context struct (§9.1) — Safe but Lifetime-Sensitive

The `QtricksContext` struct is stack-allocated within `QuickTricks` and passed by reference to sub-functions. This is inherently thread-safe (each thread has its own stack frame). However:

- The struct holds a **reference** to `pos& tpos` — this is the same `pos` that `ABsearch` mutates via `Make1`/`Undo2`. Since `QuickTricks` is called *between* Make/Undo (not concurrently), this is safe.
- Do not cache pointers to `QtricksContext` beyond the lifetime of the `QuickTricks` call.

### Summary

| Risk Level | Optimisations |
|---|---|
| **No risk** | §1, §2, §4, §5, §6, §7, §8.5, §9.3, §9.4, §9.5, §9.6, §9.7 |
| **Low risk** (mechanical refactor) | §3, §8.4, §9.1 |
| **Medium risk** (requires auditing access patterns) | §8.1, §8.3, §9.2 |
| **High risk** (concurrency semantics change) | §8.2 |

The SoA transposition table (§8.2) is the only optimisation that changes concurrency semantics. The depth-local scratch pad (§8.3) is the most invasive refactor but is thread-safe by construction — the risk is purely in getting all call sites updated consistently.
