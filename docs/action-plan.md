...
- the supported public alpha-mu include surface lives in `include/alpha_mu/`,
- the shared production data structures live in `src/alpha_mu_core.h`,
- Pareto-front / toy-search helpers live in `src/alpha_mu_front.cpp`,
- information-state and world-construction helpers live in
  `src/alpha_mu_worlds.cpp`,
- bridge-state and legality/search helpers live in `src/alpha_mu_bridge.cpp`,
- the bridge search / TT core lives in `src/alpha_mu_core.cpp`,
- the regression bundles remain isolated in
